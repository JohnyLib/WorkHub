...
import { updateShortWorkAction } from '@/lib/supabase/actions'
...
interface EditShortWorkClientProps {
  id: string
  user: any
  listing: any
}

export default function EditShortWorkClient({ id, user, listing }: EditShortWorkClientProps) {
  const router = useRouter()
  
  // Form State initialized instantly from server-fetched props
  const [serviceType, setServiceType] = useState(listing.service_type || 'Сантехника')
  ...
  const [budgetType, setBudgetType] = useState(listing.budget_type || 'fixed')
  const [budgetAmount, setBudgetAmount] = useState(listing.budget_amount ? String(listing.budget_amount) : '')
  const [paymentMethod, setPaymentMethod] = useState(listing.payment_method || 'cash')
  
  const [urgency, setUrgency] = useState(listing.urgency || 'ASAP')
  const [preferredDate, setPreferredDate] = useState(listing.preferred_date || '')
  const [preferredTime, setPreferredTime] = useState(listing.preferred_time || 'anytime')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const res = await updateShortWorkAction(id, {
      ...
      if (res.success) {
        ...
        toast.error(res.error || 'Ошибка при обновлении заявки')
      }
    } catch (err) {
      console.error(err)
      ...
      <div className="flex-1 min-w-0 max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-4 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-500 transition-all disabled:opacity-50 min-h-[52px] flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20 hover:-translate-y-0.5 cursor-pointer text-base"
          >
